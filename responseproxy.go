...
// ResponseWriterProxier wraps a http.ResponseWriter and allows it
// to be inspected by middlewares.
...
func NewResponseProxy(w http.ResponseWriter) ResponseWriterProxier {
	if o, ok := w.(ResponseWriterProxier); ok {
		return o
	}
	return newResponseProxy(w)
}

//go:generate go run ./gen/gen.go -output permutation.go
